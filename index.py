...
import pygame, sys, random, dataGame
...
def hangman():
    ...
    while play == True:
        ...
        text = pygame.font.Font("freesansbold.ttf", 40)
        textSurf = text.render("Hang Man For Kids", True, black)
        textRect = textSurf.get_rect()
        textRect.topright = (800, 200)
        screen.blit(textSurf, textRect)

        u1text = pygame.font.Font("freesansbold.ttf", 20)
        u1textSurf = u1text.render("Tran Thanh The - B20DCCN664", True, black)
        u1textRect = u1textSurf.get_rect()
        u1textRect.topright = (800, 260)
        screen.blit(u1textSurf, u1textRect)

        u2text = pygame.font.Font("freesansbold.ttf", 20)
        u2textSurf = u2text.render("Hoang Quoc Viet - B20DCCN724", True, black)
        u2textRect = u2textSurf.get_rect()
        u2textRect.topright = (800, 300)
        screen.blit(u2textSurf, u2textRect)

        button("Math", 50, 150, 300, 60, black, lightgrey, Math)
        button("Grammar", 50, 230, 300, 60,black,lightgrey, Grammar)
        button("Guess pictureData", 50, 310, 300, 60,black,lightgrey, GuessPicture)
                    
        pygame.display.update()
        clock.tick(fps)

def hangmanGame(catagory, title, point):
    global pause, pick, pickSplit, textBoxSpace, textBoxNumber
    chances = 20
    question = random.choice(catagory)
    ...
    while gamePlay == True:
        ...
        pygame.draw.rect(screen, white, [700, 20, 200, 20])

        text = pygame.font.Font("freesansbold.ttf",20)
        textSurf = text.render(("Chances: %s" % chances), False, black)
        textRect = textSurf.get_rect()
        textRect.topright = (880, 20)
        screen.blit(textSurf, textRect)

        pointText = pygame.font.Font("freesansbold.ttf",20)
        pointTextSurf = pointText.render(("Points: %s" % point), False, black)
        pointTextRect = pointTextSurf.get_rect()
        pointTextRect.topright = (880, 45)
        screen.blit(pointTextSurf, pointTextRect)

        textTitle = pygame.font.Font("freesansbold.ttf", 30)
        textTitleSurf = textTitle.render(title, True, black)
        textTitleRect = textTitleSurf.get_rect()
        textTitleRect.center = ((width/2), 120)
        screen.blit(textTitleSurf, textTitleRect)

        if "url_image" in question: 
            img = pygame.image.load(question['url_image'])
            gameDisplay.blit(img, (550, 60))
        ...
        button("Back", 20, 20, 100, 50, black, lightgrey, hangman)
...
def GamePlay(catagory, title): 
    res = True
    point = 0
    while res:
        res = hangmanGame(catagory, title, point)
        if not(res):
            endGame(point)
        point += 1
...
def Math():
    title = "Math"
    Data = dataGame.mathData()
    GamePlay(Data, title)

def Grammar():
    title = "Grammar"
    Data = dataGame.grammarData()
    GamePlay(Data, title)
  
def GuessPicture():
    title = "Guess Picture"
    Data = dataGame.guessPictureData()
    GamePlay(Data, title)
